...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['archivo']) && $_FILES['archivo']['error'] == 0 && is_uploaded_file($_FILES['archivo']['tmp_name'])) {
        ...
        if (move_uploaded_file($rutaTemporal, $rutaDestino)) {
            ...
            $archivoJSON = 'imagenes.json';     //guarda info en el json

            $contenido = file_exists($archivoJSON) ? json_decode(file_get_contents($archivoJSON), true) : []; //lee el contenido

            $contenido[] = $datos; //agrega lo nuevo

            file_put_contents($archivoJSON, json_encode($contenido, JSON_PRETTY_PRINT)); //guarda lo nuevo
